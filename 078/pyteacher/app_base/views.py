...
def protected_serve(request, path, document_root=None, show_indexes=False):
    if 'session/private-videos' not in path:
        ...
        if request.user.is_authenticated and CourseSession.objects.get(id=session_id) in request.user.profile.registered_sessions():
            return serve(request, path, document_root, show_indexes)
        else:
            messages.error(request, 'اوه! یه مشکلی پیش اومده...\nظاهرا شما روی این مورد قبلا سرمایه گذاری نکرده اید!')
...
def course_session_exercise_detail(request, course_slug, session_slug, exercise_slug):
    ...
    student = ex.chats.first().user
    ctx['submited_final_reply'] = ExerciseByStudent.objects.filter(user=student, exercise=ex).first()
...
def register_course(request, item_id, item_type):
    ...
    if item_type == 'Course' and item not in [rc.course for rc in request.user.registereditem_set.all()]:
        if item.price:
            payment(request, item_id, item_type)
...
@login_required
def payment(request, item_id, item_type):
    """ price per toman """
    ctx = {}
    if item_type not in ['Course', 'CourseSession']:
        messages.error(request, 'اوه! یه مشکلی پیش اومده...')
    ...
    if result.Status == 100:
        ...
        ctx['msg'] = 'اوه! یه مشکلی پیش اومده. ظاهرا تراکنش ناموفق بوده یا توسط کاربر لغو شده است. کد خطا: ' + \
            str(result.Status)
...
def verify(request, item_id, item_type):
    ...
    if request.GET.get('Status') == 'OK':
        ...
        ctx['msg'] = 'اوه! یه مشکلی پیش اومده. ظاهرا تراکنش ناموفق بوده یا توسط کاربر لغو شده است.'
    return render(request, 'verify-payment.html', ctx)
