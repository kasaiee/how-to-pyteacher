...
def add_ticket(request):
    ...
    if topic:
        new_ticket = Ticket(user=user, topic=topic)
        new_ticket.save()
        return Response(
            {
                "ticket_id": new_ticket.id,
                "redirect_to": new_ticket.get_absolute_url(),
                "message": "در اولین وقتی که بتونم، پاسخ تیتک رو میدم! ممنون که وقت گذاشتی..."
            },
            status=status.HTTP_200_OK)
    return Response(
        {"warning": "اوه! یه مشکلی پیش اومده..."},
...
def send_ex_rate(request, ex_id):
    if not request.user.is_superuser:
        return Response(
            ...
            status=status.HTTP_200_OK)
    ex_rate = request.POST.get('rate')
    student_id = request.POST.get('student')
    try:
        student = User.objects.get(id=student_id)
        ex = CourseSessionExercise.objects.get(id=ex_id)
        try:
            ex_by_student = ExerciseByStudent.objects.get(user=student, exercise=ex)
            ex_by_student.rate = ex_rate
            ex_by_student.save()
        except ExerciseByStudent.DoesNotExist:
            ExerciseByStudent.objects.create(user=student, exercise=ex, rate=ex_rate)
        return Response(
            {"message": "تمرین با موفقیت امتیاز داده شد!"},
            status=status.HTTP_200_OK)
