...
def edit_profile(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            ctx['form'] = form
    elif request.user.profile.birth_date:
        bd_dict = {
            'year': request.user.profile.birth_date.year,
            'month': request.user.profile.birth_date.month,
            'day': request.user.profile.birth_date.day,
        }
    else:
        bd_dict = {}
        init_dict = {**request.user.profile.__dict__, **request.user.profile.user.__dict__, **bd_dict}
