...
def ticket_list(request):
    return render(request, 'ticket-list.html', {'tickets': Ticket.objects.all().order_by('-id')})
...
def registered_items(request):
    ctx = {}
    ctx['registered_items'] = RegisteredItem.objects.filter(user=request.user).order_by('-id')
