...
def payment(request, item_id, item_type):
    ...
    if item_type == 'Course':
        ...
        price = item.price
    CallbackURL = 'http://127.0.0.1:8000' + reverse('app-base:verify', args=(item_id, item_type))  # Important: need to edit for realy server.
...
def verify(request, item_id, item_type):
    ...
    if request.GET.get('Status') == 'OK':
        ...
        if result.Status == 100:
            ...
            ctx['msg'] = 'با توجه به محدودیت های شاپرک، امکان پرداخت با رقم درخواست داده شده میسر نیست! کد خطا: ' + str(result.Status)
