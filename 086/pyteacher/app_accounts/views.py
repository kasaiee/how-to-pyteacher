...
def last_ex(request):
    ...
    ctx['all_chats'] = all_chats = Chat.objects.filter(
        user__groups__name='students', content_type=ct).order_by('-create_datetime')
    for chat in all_chats:
        chat.abs_url = chat.content_object.get_absolute_student_reply_url(chat.user.id)
